...
export function battleReducer(state: BattleState, action: BattleAction): BattleState {
  ...
  if (action.type === 'play') {
    ...
    // Story cases require the selected evidence chain, not repeated use of one strong card.
...
export type EvidenceInput = { id: string; type?: string; credibility: number };
export type BattleCase = { levelId: number; evidence: EvidenceInput[]; keyEvidenceIds: string[]; source?: unknown };

export function evidenceStats(item: EvidenceInput, key: boolean, levelId: number) {
  const power = ['document', 'image', 'img', 'payment'].includes(item.type || 'document') ? 3 : 2;
  const credibility = item.credibility >= 9 ? 2 : item.credibility >= 7 ? 1 : 0;
  const value = (key ? 3 : 1) + power + credibility;
  const cost = levelId === 1
    ? Math.max(2, Math.min(4, Math.ceil(value / 3)))
    : Math.max(2, Math.min(6, Math.ceil(value / 2)));
  return { value, cost };
}

export function opponentHealth(demo: BattleCase) {
  const cards = demo.evidence.map((item) => ({
    key: demo.keyEvidenceIds.includes(item.id),
    ...evidenceStats(item, demo.keyEvidenceIds.includes(item.id), demo.levelId),
  }));
  const health = cards.sort((a, b) => Number(b.key) - Number(a.key) || b.value - a.value)
    .slice(0, HAND_SIZE).reduce((sum, card) => sum + card.value, 0);
  if (demo.source) {
    const keyPower = cards.filter((card) => card.key).reduce((sum, card) => sum + card.value, 0);
    const supportPower = cards.filter((card) => !card.key).map((card) => card.value);
    return supportPower.length ? keyPower + Math.min(...supportPower) : health;
  }
  return demo.levelId === 1 ? Math.min(20, health) : health;
}

// Both prediction and replay derive draws from the server-issued seed.
export function actionSeed(seed: number, actionIndex: number) {
  return (seed + actionIndex + 1) >>> 0;
}
